...
The primary artifact is a per-session analysis containing zero or more **slugs**. A slug is one evidence-backed suspected failure or avoidable-work episode. Cross-session patterns come later by grouping slugs by repository.
...
When slugs overlap, session totals must use the union of implicated event IDs so the same tokens or time are not counted twice.
...
- session outcome;
- number of slugs caught;
- measured waste, explicitly noting unavailable coverage;
- the strongest slug and its replay test;
