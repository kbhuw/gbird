...
export function assertRepoReport(input: RepoReportInput, value: unknown): asserts value is RepoFailureReport {
  ...
  const expected = new Map<string, { evidence: Set<string>; title: string; agent: string }>();
...
export function createCodexRepoReporter(options: RepoReporterOptions = {}): AnalyzeRepo {
  ...
  return async (input: RepoReportInput): Promise<RepoFailureReport> => {
    ...
    const prompt = [
      ...
      "Be concise: use short titles, one brief summary, one brief condition, one brief guardrail, and a repro prompt under 80 words.",
      ...
      "The failure and success conditions must be observable in the fresh session trace.",
    ...
    try {
      ...
      const parsed = JSON.parse(fs.readFileSync(resultPath, "utf8")) as RepoFailureReport;
      ...
      for (const failure of parsed.failures) {
        failure.occurrences = failure.occurrences.map((occurrence) => {
          const source = sources.get(insightKey(occurrence.session_id, occurrence.insight_id));
      ...
      const detail = error && typeof error === "object" && "stderr" in error
        ...
        : "";
      throw new Error(detail ? `Repo report failed: ${detail}` : `Repo report failed: ${error instanceof Error ? error.message : String(error)}`);
    } finally {
